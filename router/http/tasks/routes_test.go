...
func TestUpdateApi(t *testing.T) {
	...
	t.Run("test update api not have database", func(t *testing.T) {
		task := domains.Task{
			ID:          "4f19cbbc-8c2c-49dd-b48a-eabafb6ab7f2",
			Title:       "test",
			Description: "test",
		...
		taskServiceMock := new(mocks.TaskServiceMock)
		taskServiceMock.On("GetTaskByID", mock.AnythingOfType("string")).Return(nil, fmt.Errorf("NotFound"))
		taskServiceMock.On("UpdateTask", mock.AnythingOfType("*tasks.Task"), mock.AnythingOfType("string")).Return(&task, nil)
		gin := gin.New()
		rec := httptest.NewRecorder()
		handler := tasks.TaskHandler{
...
func TestDeleteApi(t *testing.T) {
	...
	t.Run("test delete error", func(t *testing.T) {
		...
		taskServiceMock.On("DeleteTask", mock.AnythingOfType("string")).Return(fmt.Errorf("Hello World"))
		gin := gin.New()
		rec := httptest.NewRecorder()
		handler := tasks.TaskHandler{
			Service: taskServiceMock,
